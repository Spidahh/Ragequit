...
2. **[L] Rig** — legare ogni layer allo STESSO `THREE.Skeleton` del corpo base (stop ai boneInverses propri);
   maschera per-classe `baseVisibleMeshes` (Peasant: solo Face/Eyes/Eyebrows; Ranger: nascondi base);
   togliere il cappuccio duplicato; poi cancellare il polygonOffset-cerotto.
...
4. **[M] Facce** — spessore outline per-mesh (clamp su bbox) o escludere occhi/sopracciglia; regola cappuccio
   tiene viso visibile nelle cowl aperte (si fonde con la maschera della Fase 2).
