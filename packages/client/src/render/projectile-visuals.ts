  ...
  function renderFrame(proj: Map<string, SchemaProjectile>, now: number, dbgProj: HTMLElement): void {
    proj.forEach((p, id) => {
      ...
      if (!vis) {
        ...
        scene.add(trail)
        const buf = new Float32Array(TRAIL_LEN * 3)
        for (let i = 0; i < TRAIL_LEN; i++) { buf[i * 3] = p.x; buf[i * 3 + 1] = p.y; buf[i * 3 + 2] = p.z }
        vis = { mesh, trail, trailBuf: buf, trailCount: 0, lastPos: new THREE.Vector3(p.x, p.y, p.z), lastAt: now, kind }
