...
describe('classMechanicView', () => {
  it('draws Fury as pips out of the server maximum', () => {
    const v = classMechanicView(state({ classId: 'breaker', furyStacks: 3 }))
  ...
  it('flags the surge, which is the thing the tank is actually waiting for', () => {
    const v = classMechanicView(state({ classId: 'breaker', furyNextMeleeIsSurge: true }))
    expect(v).toMatchObject({ flag: 'SURGE' })
  })

  it('never shows more pips than exist, whatever the server sends', () => {
    const v = classMechanicView(state({ classId: 'breaker', furyStacks: 99 }))
    expect(v).toMatchObject({ filled: FURY_MAX_STACKS })
    expect(classMechanicView(state({ classId: 'drift', flowStacks: -4 }))).toMatchObject({
      filled: 0,
  ...
  it('puts the momentum mark at the real bow-bonus threshold', () => {
    const v = classMechanicView(state({ classId: 'talon', momentum: 30 }))
  ...
  it('calls out momentum only once the threshold is actually reached', () => {
    expect(
      classMechanicView(state({ classId: 'talon', momentum: MOMENTUM_BOW_BONUS_THRESHOLD - 1 })),
    ).toMatchObject({ flag: '' })
    expect(
      classMechanicView(state({ classId: 'talon', momentum: MOMENTUM_BOW_BONUS_THRESHOLD })),
  ...
  it('is not armed with no element at all', () => {
    expect(
      classMechanicView(
        state({ classId: 'warden', risonanzaElement: '', risonanzaArmedUntilTick: 9999 }),
      ),
...
describe('renderClassMechanic', () => {
  ...
  it('renders one dot per stack, filled and empty', () => {
    const host = el()
    renderClassMechanic(host, classMechanicView(state({ classId: 'breaker', furyStacks: 2 })))
  ...
  it('hides itself rather than leaving a stale strip on screen', () => {
    const host = el()
    renderClassMechanic(host, classMechanicView(state({ classId: 'breaker', furyStacks: 4 })))
  ...
  it('fills the momentum bar to the right width', () => {
    const host = el()
    renderClassMechanic(host, classMechanicView(state({ classId: 'talon', momentum: 50 })))
...
describe('momentum threshold notch', () => {
  ...
  it('paints the notch at the threshold, not at the fill', () => {
    ...
    renderClassMechanic(host, classMechanicView(state({ classId: 'talon', momentum: 20 })))
