...
export class SoundEngine {
  ...
  /**
   * Firing the bow or the staff. Both were completely SILENT: the sword had
   * playSwing() on the input frame, but the two ranged weapons only ever made
   * a sound if the shot happened to hit something. Played locally the moment
   * the input is sent, so the shot is audible even when it misses.
   *
   * Bow = a short broadband string snap; staff = a quick rising zap. Both sit
   * below 2 kHz so they never mask the hit-confirm band.
   */
  playWeaponFire(weapon: 'bow' | 'staff'): void {
    if (this._muted) return
    const ac = this.ac,
      out = this.out
    const gain = ac.createGain()
    gain.connect(out)
    if (weapon === 'bow') {
      // Filtered noise burst — the thwip of a released string.
      ...
      this._pitch(src)
      src.start()
      src.stop(ac.currentTime + 0.16)
      return
    }
    const osc = ac.createOscillator()
    osc.type = 'triangle'
    osc.frequency.setValueAtTime(420, ac.currentTime)
    osc.frequency.exponentialRampToValueAtTime(1150, ac.currentTime + 0.09)
