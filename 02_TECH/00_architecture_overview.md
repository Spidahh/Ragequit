...
packages/client/dist/  # Vite production output for Cloudflare Pages
```
...
| Client app/input/render/HUD    | `packages/client/src/`; game input controller in `input/game-input.ts`, radial wheel controller in `input/radial-wheels.ts`, mouse sensitivity in `input/sensitivity.ts`, cast/fire/weapon dispatcher in `input/cast-dispatcher.ts`, public server probe in `net/server-health.ts`, HUD drag/resize in `hud/hud-drag.ts`, hotbar/cooldown strip in `hud/cd-strip.ts`, mechanic readout in `hud/ability-readout.ts`, combat feed in `hud/combat-feed.ts`, self-player HUD in `hud/self-hud.ts`, ability fail / server toast in `hud/ability-fail-hud.ts`, hit feedback (hitmarker/dir hit/damage popup) in `hud/hit-feedback.ts`, bow charge / parry ring / round timer / vignettes in `hud/combat-overlay-hud.ts`, status applied/expired vignette flash in `hud/status-overlay.ts`, self-character emissive / player-light in `render/self-emissive.ts`, projectile visuals in `render/projectile-visuals.ts`, zone visuals in `render/zone-visuals.ts`, placement preview in `render/placement-preview.ts`, remote player visuals in `render/remote-players.ts`, arena animation in `world/arena.ts` (animateArena) |
...
| Ability readout       | `hud/ability-readout.ts`                                       | Temporary mechanic summary near crosshair on prime, placement and cast         |
| Semantic animations   | `render/character-animation.ts` + `render/character-loader.ts` | Maps class clips by gameplay role; no index-based state guessing               |
...
- The four live classes load optimized Mixamo GLBs (`paladin`, `erika`,
  `vampire`, `ninja`, about 1.8-2.5 MB each). Modular .gltf + .bin characters
  remain fallback/source assets; `UAL1_Standard.glb` supplies animation clips.
